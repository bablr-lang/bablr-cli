...
export const writePrettyCSTMLStrategy = (tags, options = freeze({})) => {
...
export const writeCSTMLStrategy = (tags, options = freeze({})) =>
    ...
    if (instr.type === 'Effect') {
      ...
      if (verb === 'write') {
        ...
        if (isString(value)) {
          ...
          for (let tag_ of streamFromTree(value)) {
            ...
            if (tag.type === OpenNodeTag) {
              ...
              if (tagName === Symbol.for('Alternative')) {
                ...
                currentName === Symbol.for('BindingTag') ||
                currentName === Symbol.for('BindingMatcher')
              ) {
                yield buildAnsiPushEffect('bold gray');
            ...
            if (tag.type === CloseTag || (tag.type === OpenNodeTag && tag.value.selfClosing)) {
              names = names.pop();
...
export const generateOutput = (tags, options = freeze({})) => {
