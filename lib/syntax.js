...
function* __higlightStrategy(context, tokens) {
  ...
  for (;;) {
    ...
    if (tag.type === OpenNodeTag) {
      ...
      if (
        tagType === 'LiteralTag' ||
        (tagType === 'String' && currentRef.name === 'intrinsicValue')
      ) {
        if (tagType === 'LiteralTag' || currentType === 'OpenNodeTag') {
          yield buildAnsiPushEffect('bold green');
        } else if (currentType === 'OpenNodeMatcher') {
